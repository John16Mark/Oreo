...
gcc prueba2.c -o prueba2.exe lib/disenio.c lib/ascii_art.c lib/menu.c lib/TADLista/TADListaDL.c
...
void *procesoDescompresion(void *arg)
{
	...
	j = 0;
	int n_bits_code = n_bytes * 8 - bits_sobrantes;		// Numero de bits
	valArbol aux;

	for(i = 0; i < n_bits_code; i++){
		
		// Caso Pone_1
		if(CONSULTARBIT(A[i/8], 7 - i%8) == 1){
		...
		// Si ya llegó
		if(aux.val == true){
			ASalida[j] = aux.c;			// Guarda el caracter en el arreglo de salida
...
valArbol validaCaracter(posicion n, unsigned char byte_aux, int bits_consultados){
	...
	if(n->ramaIzq == NULL && n->ramaDer == NULL){
		aux.c = n->e.c;
		aux.val = true;
		return aux;
	}
